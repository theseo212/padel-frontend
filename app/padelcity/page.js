...
export default function PaginaPadelCity() {
  ...
  useEffect(() => {
    async function caricaCampionatiDisponibili() {
      try {
        const risposta = await fetch(`${API_URL}/padelcity/campionati/pubblico`);
  ...
  async function verificaNumeroConosciuto() {
    ...
    try {
      const risposta = await fetch(
        `${API_URL}/padelcity/utenti/profilo?whatsapp_numero=${encodeURIComponent(whatsappCompleto)}`
      );
      const dati = await risposta.json();
      ...
      if (dati.esiste) {
        ...
        // prima volta qui su PadelCity: precompiliamo nome/cognome/
  ...
  async function inviaIscrizione(event) {
    ...
    setInviando(true);
    try {
      const risposta = await fetch(`${API_URL}/padelcity/iscrizione`, {
        method: 'POST',
  ...
  async function confermaOtp(event) {
    ...
    setInviando(true);
    try {
      const risposta = await fetch(`${API_URL}/padelcity/iscrizione/valida-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <main className="pagina">
        <div className="sezione">
          ...
          <div className="riepilogo-richiesta">
            <p>
              Hai scelto di giocare i campionati settimanali di <strong>PadelCity</strong>:{' '}
  ...
  return (
    <>
      <section className="hero-padelcity">
        <div className="hero-padelcity-immagine-contenitore">
          <img src="/anna_padelcity.png" alt="Anna" className="hero-immagine" />
        </div>
      </section>

      <main className="pagina" id="form-iscrizione-padelcity">
      <div className="intestazione">
        <h1>
          <img src="/racchetta-icona.svg" alt="" width="24" height="24" style={{ verticalAlign: 'middle', marginRight: '6px' }} />
          AnnaPadel per il PadelCity
        </h1>
        <p>Ciao sono Anna! Dimmi quando vuoi giocare e gestirò le tue iscrizioni ai campionati settimanali PadelCity, avvisandoti su WhatsApp.</p>
      </div>
          ...
          <div className="campo">
            ...
            {trovatoNelGenerico && !profilo && (
              <p className="profilo-riconosciuto">
                ...
                scegliere lato di gioco e campionati qui sotto per completare l'iscrizione a PadelCity.
              </p>
            )}
          </div>
        </section>
        ...
        <section className="sezione">
          ...
          <div className="campo">
            ...
            {!caricandoCampionati && !erroreCampionati && (
              <div className="griglia-giorni-padelcity">
        ...
        {!profilo && (
          <div className="accettazione-legale">
            <label>
              ...
              />
              {' '}Accetto i <a href="/padelcity/termini" target="_blank" rel="noopener noreferrer">Termini e Condizioni</a>
            </label>
            <label>
              <input
                type="checkbox"
              ...
              />
              {' '}Accetto la <a href="/padelcity/privacy" target="_blank" rel="noopener noreferrer">Privacy Policy</a>
            </label>
          </div>
        )}
